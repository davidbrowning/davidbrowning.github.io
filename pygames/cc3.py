...
class Game:
        ...
        for y in range(GRID_SIZE):
            for x in range(GRID_SIZE):
                if self.grid[y][x]:
                    ...
                    if candy.popping:
                        color = (*color, candy.alpha)
                    else:
                        color = (*color, 255)
                    size = int((CELL_SIZE//2 - 5) * candy.scale)
                    pygame.draw.circle(candy_surface, color,
                                     (int(candy.x + CELL_SIZE//2), 
                                      int(candy.y + CELL_SIZE//2)), 
                                     size)
                    pygame.draw.circle(candy_surface, (0, 0, 0, candy.alpha),
                                     (int(candy.x + CELL_SIZE//2), 
                                      int(candy.y + CELL_SIZE//2)), 
                                     size, 2)
    ...
    def update_animations(self):
        # Handle popping and falling animations
        for y in range(GRID_SIZE):
            for x in range(GRID_SIZE):
                if self.grid[y][x]:
                    candy = self.grid[y][x]
    ...
    def check_rendering(self):
        # Basic rendering verification: ensure no candies are out of bounds
        for y in range(GRID_SIZE):
            for x in range(GRID_SIZE):
                if self.grid[y][x]:
                    candy = self.grid[y][x]
...
def main():
    ...
    while running:
                    ...
                    if (abs(sel_x - new_x) + abs(sel_y - new_y) == 1):
                        if (0 <= sel_y < GRID_SIZE and 0 <= sel_x < GRID_SIZE and
                            ...
                            if matches:
                                ...
                                # Wait for popping to finish before next steps
                            else:
        ...
        # Step 1: Update animations (including popping)
        game.update_animations()

        # Step 2: Apply gravity only after all popping is done
        ...
        # Step 3: Draw and verify rendering
        game.draw()
